...
`;
  ...
  if (isCopyMode) {
    console.log(
      chalk.cyan('\n📋 Running in manual copy mode for prompt generation.'),
    );
  ...
  const promptSpinner = new Spinner();
  promptSpinner.start(`Asking ${model} to craft your prompt template...`);
  ...
  try {
    ...
    promptSpinner.succeed('Prompt draft received');
    ...
    promptSpinner.fail('Prompt generation failed');
