...
impl LlmClient {
    ...
    pub fn chat_completion_with_thinking(
        ...
        let mut response = self.send_chat_request(body, true)?;
            ...
            let n = reader.read(&mut buf).map_err(|e| {
                if is_timeout_err(&e) {
                    LlmError::Http(format!(
                        "stream idle timeout after {STREAM_READ_TIMEOUT_SECS}s"
                    ))
                } else {
    ...
    fn send_chat_request(
        ...
        if streaming {
            request = request
                .config()
                .timeout_recv_body(Some(Duration::from_secs(STREAM_READ_TIMEOUT_SECS)))
